<template>
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <br>
                <button @click="selectedComponent = 'appBlue'" class="btn btn-primary">Load Blue Template</button>
                <button @click="selectedComponent = 'appGreen'" class="btn btn-success">Load Green Template</button>
                <button @click="selectedComponent = 'appRed'" class="btn btn-danger">Load Red Template</button>
                
                <hr>

                <!-- Dynamic -->
                <component :is="selectedComponent">
                    <p>Here's the content</p>
                </component>

                <!-- Actual switching content -->
                <app-blue v-if="selectedComponent == 'appBlue'">
                    <h2>Here's the Blue Template</h2>
                </app-blue>
                
                <app-green v-if="selectedComponent == 'appGreen'">
                    <h2>Here's the Green Template</h2>
                </app-green>

                <app-red v-if="selectedComponent == 'appRed'">
                    <h2>Here's the Red Template</h2>
                </app-red>
            </div>
        </div>
    </div>
</template>

<script>
    import Blue from './components/Blue.vue';
    import Green from './components/Green.vue';
    import Red from './components/Red.vue';

    export default {
        data: function() {
            return {
                selectedComponent: "appBlue"
            };
        },
        components: {
            appBlue: Blue,
            appGreen: Green,
            appRed: Red
        }
    }
</script>

<style>
</style>
